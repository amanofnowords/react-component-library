...
const Form = ({ formAttributes, children, formID, initialFormState = {}, onSubmit, groupErrors = false, errorMessage = 'Please check form for errors.' }) => {
    ...
    return React.createElement(GlobalContext.Provider, { value: Object.assign(Object.assign({}, state), { updateState }) },
        React.createElement("form", Object.assign({ id: formID }, formAttributes),
            children,
            (groupErrors && Object.values(state.errors).includes(true)) &&
                React.createElement("div", null,
                    React.createElement("p", { className: "ge-errorMessage" }, errorMessage)),
            React.createElement("button", { onClick: e => { localOnSubmit(e); } }, "Submit")));
};
...
const TextInput = ({ componentID, children, value = '', placeholder = 'Please Enter Value', label = "Text Input:", name = label, containerClassName = '', regexType = 'personName', customRegex, errorMessage = 'Error: Please Check Value', onChangeCallback = undefined, inputAttributes, labelAttributes, errorMessageAttributes, validate = true, optional = false, }) => {
    ...
    if (isThisASmartComponent) {
        ...
        var [inputValue, setInputValue] = React.useState(value);
        if (validate === true && (renderCount <= 1 && inputValue === '')) {
            var [errorExist, setError] = React.useState(false);
        }
        else if (validate === true) {
            var [errorExist, setError] = React.useState(!isThisValueValid.default(inputValue, regex));
        }
    }
    ...
    return (React.createElement("div", { className: `text-input ${containerClassName} ${errorExist ? 'error' : ''}`, onBlur: e => onInputValueChange(e) },
        label !== null && React.createElement("label", Object.assign({ htmlFor: componentID }, labelAttributes), label),
        React.createElement("input", Object.assign({ type: "text", id: componentID, placeholder: placeholder, value: inputValue }, inputAttributes, { onChange: (e) => {
                onInputValueChange(e);
            } })),
        children,
        (errorExist === true && !Context.groupErrors) && (React.createElement("p", Object.assign({ className: "errorMessage" }, errorMessageAttributes), errorMessage))));
};
